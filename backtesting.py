...
class Investing:
    ...
    def expansion(self, estimate_price):
        # Index Range
        ind_init = list(pack.market_open).index(self.portfolio.index[0])
        ind_end = list(pack.market_open).index(self.portfolio.index[-1])
        ind_E = pack.market_open[ind_init:ind_end+1]

        # Generate Empty DataFrame for expansion
        plate_portfolio = pd.DataFrame(index=ind_E, columns=self.portfolio.columns)
        plate_wallet = pd.DataFrame(index=ind_E, columns=self.wallet.columns) 
        
        # Expansion Function for high speed Converting
        def _expansion_portfolio(row):
            if row.name in self.portfolio.index:
                row = self.portfolio.loc[row.name]
            return row
        def _expansion_wallet(row):
            if row.name in self.wallet.index:
                row = self.wallet.loc[row.name]
            return row
...
def backtest(port_weight, deposit, buy='open', sell='open', asset_name='stock', fee_rate=0.00015, tax_rate=0.003, set_from=False, set_to=False):
    ...
    # ********** Out of Range에 대한 부분 차후 업데이트 필요 **********
...
if __name__ == "__main__":
    ...
    no_cost = backtest(strategy_sample['daily_strategy01'], 100000000, fee_rate=0., tax_rate=0., asset_name='test_WithCost', set_from='2019-01-01', set_to='2019-09-30')
    yes_cost = backtest(strategy_sample['daily_strategy01'], 100000000, asset_name='test_WithoutCost', set_from='2019-01-01', set_to='2019-09-30')
